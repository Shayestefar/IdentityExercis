...
namespace Simple.Controllers
{
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> ResetPasswordAsync(string email, string code, string password, CancellationToken cancellationToken)
        {
            var user = await UserManager.FindByEmailAsync(email);
            if (user == null)
            {
                ModelState.AddModelError(string.Empty, "توکن نامعتبر.");
                return View();
            }
